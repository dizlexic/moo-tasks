  ...
  <div class="max-w-4xl mx-auto px-6 py-12">
    ...
    <div v-else-if="accountInfo" class="space-y-8">
      ...
      <section class="bg-white dark:bg-surface-card rounded-3xl p-8 shadow-sm border border-gray-200 dark:border-surface-border">
        <h2 class="text-xs font-black uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 mb-6 flex items-center gap-2">
          <span class="w-1.5 h-1.5 rounded-full bg-neon-cyan"></span>
          Profile Information
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1 ml-1">Full Name</label>
          ...
          <div>
            <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1 ml-1">Email Address</label>
            <div class="text-lg font-bold text-gray-900 dark:text-white px-4 py-3 bg-gray-50 dark:bg-surface-raised/50 rounded-2xl border border-gray-100 dark:border-surface-border/50">
              {{ accountInfo.email }}
            </div>
          </div>
      ...
      <section class="bg-white dark:bg-surface-card rounded-3xl p-8 shadow-sm border border-gray-200 dark:border-surface-border">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-xs font-black uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 flex items-center gap-2">
        ...
        <div class="space-y-6">
          ...
          <div v-if="accountInfo.hasAccountToken || mcpToken" class="space-y-4">
            <div v-if="mcpToken" class="p-6 bg-neon-green/5 border border-neon-green/20 rounded-2xl">
              <label class="block text-[10px] font-black uppercase tracking-widest text-neon-green mb-2 ml-1">Your New Account Token</label>
              <div class="flex items-center gap-3">
                ...
                <button 
                  @click="copyToken"
                  class="px-4 py-3 rounded-xl bg-neon-green text-gray-900 font-bold text-xs uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-lg shadow-neon-green/20"
                >
            ...
            <div class="flex flex-wrap gap-3">
              <button @click="generateToken" :disabled="tokenLoading" class="text-[10px] font-bold uppercase tracking-widest px-5 py-2.5 rounded-xl bg-neon-purple/10 text-neon-purple border border-neon-purple/20 hover:bg-neon-purple/20 transition-all disabled:opacity-50">
                🔄 Regenerate Token
              </button>
              <button @click="revokeToken" :disabled="tokenLoading" class="text-[10px] font-bold uppercase tracking-widest px-5 py-2.5 rounded-xl border border-neon-red/20 bg-neon-red/5 text-red-600 dark:text-neon-red hover:bg-neon-red/15 transition-all disabled:opacity-50">
                🗑 Revoke Token
              </button>
            </div>
          </div>
          ...
          <div v-else>
            <button @click="generateToken" :disabled="tokenLoading" class="text-[10px] font-bold uppercase tracking-widest px-6 py-3 rounded-xl bg-neon-purple text-white hover:bg-neon-purple/90 transition-all shadow-lg shadow-neon-purple/20 active:scale-95">
